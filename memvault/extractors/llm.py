...
DEFAULT_CACHE_DIR = Path.home() / ".cache" / "memvault" / "models"
...
class GemmaExtractor(BaseExtractor):
    ...
    def _ensure_model(self) -> str:
        try:
            ...
            raise ImportError("Install huggingface-hub: pip install memvault[llm]")

        DEFAULT_CACHE_DIR.mkdir(parents=True, exist_ok=True)
        path = DEFAULT_CACHE_DIR / MODEL_FILE
        if not path.exists():
            print(f"[memvault] Downloading {MODEL_FILE} (~800MB, one-time)...")
            hf_hub_download(
                repo_id=MODEL_REPO,
                filename=MODEL_FILE,
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise ImportError("Install llama-cpp-python: pip install memvault[llm]")
